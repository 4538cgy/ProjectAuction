...
import com.example.project_auction.databinding.ItemLayoutTitleBinding
...
class SettingAdapter(private val items : ArrayList<SettingItem>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        when(viewType){
            title -> {
                return TitleViewHolder(DataBindingUtil.inflate(LayoutInflater.from(parent.context),R.layout.item_layout_title,parent,false))
            }
            else -> {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (items[position].type == SettingType.TITLE){
            ...
            holder.binding.itemLayoutTitleTextview.text = items[position].title.toString()
        }else{
    ...
    inner class TitleViewHolder(val binding : ItemLayoutTitleBinding) : RecyclerView.ViewHolder(binding.root){
